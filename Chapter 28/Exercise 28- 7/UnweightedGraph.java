...
Exercise 28-7
(Find a cycle) Add a new method in UnweightedGraph to find a cycle in the graph
with the following header: public List<Integer> getACycle()
...
public class UnweightedGraph<V> implements Graph<V> {
    ...
    public List<Integer> getACycle(){
        ...
        for(int i = 0; i < vertices.size(); i++) {//for loop to fill in vertices
            list.add(i);
        }
       ...
       while (!list.isEmpty()) { // check list size for this
            int v = list.get(0);//pick one, say v;
            Stack<Integer> stack = new Stack<>(); //create stack;
            stack.push(v); //push v to the stack;
            isVisited[v] = true; //mark v visited;
            list.remove(v); //remove v from list with all vertices;
            ...
            while (!stack.isEmpty()) {
                ...
                if (neighbors.get(x).size() == 0) { //if x has no neighbors
                    ...
                    for(int i = 0; i < neighbors.get(x).size(); i++){ //loop through all neighbors of x with loop variable i
                        ...
                        if (!isVisited[e.v]) { //ending vertex of e (e.v) is not visited) {
                            parent[e.v] = x; //parent[e.v] = x;
                            stack.push(e.v); //push e.v onto the stack;
                            isVisited[e.v] = true; //mark e.v visited;
                            list.remove(new Integer(v)); //remove e.v from list with all vertices; *** getting reply on this ***
                            //to why v in list.remove(new Integer(v)) works when e.v cause out of bounds ex for the second call.
                            neighbors.get(x).remove(i); //remove loop variable i from x's neighbor list;
                            break;
                        }
                        else if (e.v != parent[x]) { //if e.v is not x's parent
                            List<Integer> listTemp = new ArrayList<>(); //create an Integer ArrayList;
                            listTemp.add(e.v); //add e.v to list;
                            while (x != e.v && x != -1) {
                                listTemp.add(x); //add x to list;
