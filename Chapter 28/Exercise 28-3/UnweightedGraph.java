...
public class UnweightedGraph<V> implements Graph<V> {
    ...
    public SearchTree dfsNonRecurs(int v) {
        ...
        stack.push(v); //Push v to stack
        searchOrder.add(v);//Add v to searchOrder
        isVisited[v] = true;//Set v to visited

        while(!stack.isEmpty()) { //stack not empty
            int x = stack.peek(); //;peek a vertex from stack

            if(neighbors.get(x).isEmpty()){ //neighbor list equal 0 pop from stack
                stack.pop();
            }
            else{
                for(int i = 0; i < neighbors.get(x).size(); i++){
                    Edge e = neighbors.get(x).get(i);
                    neighbors.get(x).remove(i); //removing ending vertex from x's neighbor list
                    if(!isVisited[e.v]){ //ending vertex is not visited
                        parent[e.v] = x; //mark parent of ending vertex as x
                        stack.push(e.v); //pushing end vertex to stack
                        isVisited[e.v] = true; //mark ending vertex as visited
                        searchOrder.add(e.v); //adding to search order
                        break;
                    }
                }
            }
